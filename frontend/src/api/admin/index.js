...
// 分类管理
createCategory(data) {
  return request.post('/admin/content/content/category', data)
},
getCategoryList() {
  return request.get('/admin/content/content/category/list')
},
updateCategory(categoryId, categoryName, description = '') {
  return request.put(`/admin/content/content/category/${categoryId}`, null, {
    params: { category_name: categoryName, description }
  })
},
deleteCategory(categoryId) {
  return request.delete(`/admin/content/content/category/${categoryId}`)
},

// 知识点管理
getKnowledgeList(page = 1, size = 10) {
  return request.get('/admin/content/content/knowledge/list', {
    params: { page, size }
  })
},
getKnowledgeDetail(pointId) {
  return request.get(`/admin/content/content/knowledge/point/${pointId}`)
},
updateKnowledge(pointId, data) {
  return request.post(`/admin/content/content/knowledge/point/${pointId}`, null, {
    params: data
  })
},
deleteKnowledge(pointId) {
  return request.delete(`/admin/content/content/knowledge/point/${pointId}`)
},

// ========== 内容全局管理（完全匹配Swagger接口） ==========
// 文档管理
getDocumentList(params) {
  return request.get('/admin/content/content/document/list', { params })
},
getDocumentDetail(docId) {
  return request.get(`/admin/content/content/document/${docId}`)
},
deleteDocument(docId) {
  return request.delete(`/admin/content/content/document/${docId}`)
},
// 文档下载（特殊处理：绕过响应拦截器）
downloadDocument(docId) {
  // 直接使用原生fetch下载，避免Axios响应拦截器的问题
  const token = localStorage.getItem('token')
  const url = `/api/admin/content/content/document/${docId}/download`
