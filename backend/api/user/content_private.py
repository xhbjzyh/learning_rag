...
from fastapi import APIRouter, Depends, UploadFile, File, Form, Query, Path, BackgroundTasks
...
async def upload_private_document(
    ...
    background_tasks.add_task(
        content_private_service.parse_and_extract_points,
        db=db,
        doc_id=result.id,
        user_id=current_user.id
    )

    return success_response(data=result, msg="文档上传成功，正在提取知识点...")
